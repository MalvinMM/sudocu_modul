...
@section('title')
    <h3 class="nav-link">Edit Module {{ $module->Name }}</h3>
@endsection
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            ...
            @endif
            <form action="{{ route('updateModule', [$erp, $module->ModuleID]) }}" method="POST" id="fullForm"
                enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="mb-3">
                    <label for="namaModul" class="form-label">Nama Modul <span style="color: red;">*</span></label>
                    <input type="text" class="form-control" id="namaModul" name="Name"
                ...
                <div class="mb-3">
                    <label for="ModuleDesc" class="form-label">Deskripsi Modul <span style="color: red;">*</span></label>
                    <textarea class="form-control" style="height:100px" id="ModuleDesc" name="ModuleDesc">{{ old('ModuleDesc', $module->Description ?? '') }}</textarea>
                    @error('ModuleDesc')
                        <h6 class="form-helper" style="color:red">{{ $message }}</h6>
                    @enderror
                </div>
                <div class="mb-3">
                    ...
                    <select name="Category" id="Category" class="form-select" style="width:20%">
                        ...
                        @foreach ($categories as $category)
                            <option value="{{ $category->CategoryID }}" @if (old('Category', $module->CategoryID == $category->CategoryID)) selected @endif>
                ...
                @if (!$details->isEmpty())
                    @foreach ($details as $index => $detail)
                        <div class="card details-card">
                            ...
                            <input type="hidden" name="detailsID[]" value="{{ $detail->ModuleDetailID }}">
                        </div>
                    @endforeach
                    {{-- @else
                    <div class="card details-card">
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                <label for="sequence" class="form-label">Sequence</label>
                                <input type="hidden" class="form-control" id="sequence" name="sequence[]"
                                    value="1" style="width:10%" value="{{ old('sequence.1', 1) }}">
                                <input type="number" class="form-control" id="sequence" name="sequence[]"
                                    value="1" style="width:10%" value="{{ old('sequence.1', 1) }}" disabled>
                            </div>
                            <div class="mb-3">
                                <label for="Description" class="form-label">Deskripsi Sequence <span
                                        style="color: red;">*</span></label>
