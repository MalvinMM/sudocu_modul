...
@section('title')
    <h3 class="nav-link">Kategori Modul {{ $erp }}</h3>
@endsection
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('addModuleCategory', $erp) }}" method="POST" id="fullForm">
                @csrf
                ...
                <div class="d-flex align-items-center">
                    <div>
                        ...
                        @error('Name')
                            <h5 class="form-helper red-important">{{ $message }}</h5>
                        @enderror
                    </div>
                        ...
                        <tbody>
                            @foreach ($categories as $item)
                                ...
                                <td class="border-bottom-0  text-center justify-content-center">
                                    <form action="{{ route('delModuleCategory', [$erp, $item->CategoryID]) }}"
                                        method="POST">
                                        @method('delete')
                                        @csrf
