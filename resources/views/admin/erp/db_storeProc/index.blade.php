...
@section('title')
    <h3 class="nav-link">Database Store Procedure {{ $erp }}</h3>
@endsection
@section('content')
    ...
    <div class="card">
        <div class="card-body p-4">
            <h4 class="card-title fw-semibold mb-4">Database Store Procedure List</h4>
            {{-- {{ dd(auth()->user()->Role) }} --}}
            @if (auth()->user()->Role != 'User')
                <h5 class="fw-semibold mb-3"><a href="{{ route('addStoreProc', $erp) }}">Tambahkan Store
                        Procedure</a></h5>
            @endif

            <form action="{{ route('searchDBStoreProc', $erp) }}" method="GET" style="width:25%"
                ...
                <table class="table text-nowrap mb-0 align-middle">
                    <thead class="text-dark
                        ...
                        <tr>
                            ...
                            <th class="border-bottom-0 text-center">
                                <h4 class="fw-semibold mb-0">Nama Store Procedure</h4>
                            </th>
                            <th class="border-bottom-0 text-center">
                    ...
                    <tbody>
                        @foreach ($storeProcs as $storeProc)
                            <tr>
                                <td class="border-bottom-0 text-center">
                                    <h5 class="fw-semibold mb-1">{{ $loop->iteration }}</h5>
                                </td>
                                <td class="border-bottom-0 text-center">
                                    <h5 class="fw-semibold mb-1">{{ $storeProc->Name }}</h5>
                                </td>
                                <td class="border-bottom-0 text-center">
                                    <h5 class="fw-semibold mb-1">
                                        <pre style="font-family:var(--bs-font-sans-serif);font-size: 1.09375rem;">{{ strlen($storeProc->Description) > 75 ? substr($storeProc->Description, 0, 75) . '...' : $storeProc->Description }}</pre>
                                    </h5>
                                </td>
                                <td class="border-bottom-0 text-center justify-content-center">
                                    <div class="d-flex align-items-center gap-2 justify-content-center mb-1">
                                        <span class="badge bg-info rounded-1 fw-semibold"><a
                                                href="{{ route('detailStoreProc', ['erp' => $erp, 'id' => $storeProc->id]) }}"
